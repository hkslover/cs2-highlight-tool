...
- 广告只渲染 `main_steps_top_banner`，为纯图卡片：整块图片即点击目标，不渲染任何文本层、不使用 `v-html`（`rich_html` 不再进入前端契约，也无净化流程）。槽位高度固定 `113px`：最小窗口下素材盒为 `890×111`（约 `8:1`，同 728×90 Leaderboard 家族），广告主规格为 2 倍图 `1780×222`；窗口宽高比与素材比例的差异由「模糊背景（`cover`+`blur`）+ 前景原图（`contain`）」双层渲染吸收，不得改回用 `cover` 裁切主图。点击走外部浏览器，图片加载失败按广告失败处理并移除该条。窗口下限 `920×680`，布局按此下限设计，不在导入方式卡片区混入广告入口。
